...
async def generator_node(state: AgentState) -> AgentState:
    ...
    if not roadmap or not target_week:
         return {**state, "error": "Missing data for generation (roadmap or target_week)"}
         
    if not context_chunks and (not state.get("skip_summary") or not state.get("skip_quiz")):
         return {**state, "error": "Missing context data for generation"}
    ...
    try:
        ...
        if not state.get("skip_quiz"):
            ...
            quiz_data = [q.model_dump() for q in quiz_result.questions]
